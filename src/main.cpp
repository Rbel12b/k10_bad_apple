...
void _initXL9535() {
    ...
    Wire.write(0x06); Wire.write(0xFC);
    err = Wire.endTransmission();
    if (err) log_e("XL9535 cfg0 err=%d", err);

    // Config port 1: bit 7 = output (AMP_GAIN); bits 0-6 = input
    Wire.beginTransmission(K10_XL9535_ADDR);
    Wire.write(0x07); Wire.write(0x7F);
    err = Wire.endTransmission();
    if (err) log_e("XL9535 cfg1 err=%d", err);

    // Output port 0: all outputs low
    _xl9535P0Out = 0x00;
    Wire.beginTransmission(K10_XL9535_ADDR);
    Wire.write(0x02); Wire.write(_xl9535P0Out);
    err = Wire.endTransmission();
    if (err) log_e("XL9535 out0 err=%d", err);

    // Output port 1: all outputs low
    _xl9535P1Out = 0x00;
    Wire.beginTransmission(K10_XL9535_ADDR);
    Wire.write(0x03); Wire.write(_xl9535P1Out);
    err = Wire.endTransmission();
    if (err) log_e("XL9535 out1 err=%d", err);
}
...
void _xl9535Flush(uint8_t port) {
    ...
    if (err) log_e("XL9535 flush port%d err=%d", port, err);
}
...
void _xl9535WritePin(uint8_t pin, bool val) {
    ...
    uint8_t bit  = pin % 8;
    if (port == 0) {
        if (val) _xl9535P0Out |=  (1 << bit);
        else     _xl9535P0Out &= ~(1 << bit);
    } else {
        if (val) _xl9535P1Out |=  (1 << bit);
        else     _xl9535P1Out &= ~(1 << bit);
    }
    _xl9535Flush(port);
}
...
void setup()
{
  ...
  Wire.setTimeOut(100);  // belt+suspenders: apply to running driver
  _initXL9535();   // must be first: controls backlight, camera reset, amp
